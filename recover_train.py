...
# y_train = np.tile(y_train, (100, 1, 1, 1))
# y_train += np.random.random(y_train.shape) * 0.05
...
# )
#
# X_train = np.asarray([random_brightness(x, (0.95, 1.05)) for x in X_train])
# X_train = np.asarray([random_zoom(x, (0.95, 1.05)) for x in X_train])
# X_train = np.asarray([random_rotation(x, 3) for x in X_train])
...
# autoencoder_with_skip = Generator()
